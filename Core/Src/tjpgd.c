...
)
{
	...
	if (JD_FORMAT == 1) {
		...
		do {
			...
			w |= *s++ >> 3;				/* -----------BBBBB */
			//*d++ = w;
			// Swap bytes based on https://github.com/Bodmer/TJpg_Decoder/blob/master/src/tjpgd.c#L945
...
)
{
	...
	for (;;) {				/* Parse JPEG segments */
		...
		if (len <= 2 || (marker >> 8) != 0xFF) return JDR_FMT4; // JDR_FMT1
		...
		case 0xDA:	/* SOS - Start of Scan */
			...
			if (!jd->width || !jd->height) return JDR_FMT5;	/* Err: Invalid image size */  // JDR_FMT1
			...
			for (i = 0; i < jd->ncomp; i++) {
				...
				if (!jd->huffbits[n][0] || !jd->huffbits[n][1]) {	/* Check huffman table for this component */
					return JDR_FMT6;					/* Err: Nnot loaded */  // JDR_FMT1
				}
				if (!jd->qttbl[jd->qtid[i]]) {			/* Check dequantizer table for this component */
					return JDR_FMT7;					/* Err: Not loaded */  // JDR_FMT1
				}
			}

			/* Allocate working buffer for MCU and pixel output */
			n = jd->msy * jd->msx;						/* Number of Y blocks in the MCU */
			if (!n) return JDR_FMT8;					/* Err: SOF0 has not been loaded */  // JDR_FMT1
